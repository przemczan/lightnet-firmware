...
| `test_link_arq` | [`test/test_link_arq/test_main.cpp`](https://github.com/przemczan/lightnet-firmware/blob/master/test/test_link_arq/test_main.cpp) | Link-ARQ pure pieces (`Core/Relay/LinkArq.hpp`) — the `PACKET_LINK_ACK` wire entry, the `Protocol::isLinkAckedType()` policy table, the `LinkAckMatcher` micro-framer a sender's ack window drains bytes through, and the `LinkDedup` duplicate window a receiver consults before dispatching |
...
| `test_trunk_frame_receiver` | [`test/test_trunk_frame_receiver/test_main.cpp`](https://github.com/przemczan/lightnet-firmware/blob/master/test/test_trunk_frame_receiver/test_main.cpp) | `Core/Relay/TrunkFrameReceiver` — `PacketFramer` plus an inter-byte idle-gap reset for the controller's un-muxed trunk RX paths: a stray leading byte (e.g. panel power-up line noise) must not make the framer latch onto a phony type/size and swallow the real frames that follow |
