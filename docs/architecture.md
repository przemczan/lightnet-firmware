...
```
src/main.cpp
├── #ifdef LIGHTNET_TARGET_CONTROLLER → src/controller/main.h   (ESP8266/ESP32)
└── #else                             → src/panel/main.hpp      (ATmega)
```
...
All firmware code lives under `lib/Lightnet/`. Files marked *(scenes)* exist only on the `scenes` branch.
...
| `LightnetPinger` | GPIO ping pulses. `HANDSHAKE` = 500 µs, `DONE` = 2000 µs. Owns an 8-entry ring buffer. `busIsDisabled` is **static (shared)** — set during any ping so all pingers ignore ISR samples while a pulse is being driven |
| `Protocol` | All I²C packet structs (`__packed__`), CRC validation, `setPacketMeta()` |
| `LightnetConfig` *(scenes)* | Cross-cutting constants: `LIGHTNET_MAX_PANELS=100`, `PALETTE_STOPS=16`, `BASE_COLORS_COUNT=3` |
| `ColorRef` *(scenes)* | 4-byte tagged union: `kind=0` inline RGB, `kind=1` palette position, `kind=2` base-color slot |
| `Palette` *(scenes)* | `GradientStop` struct (pos+RGB, 4 B) and `samplePalette()` linear interpolation. No FastLED dependency |
...
| `RGBController` | FastLED (ATmega328) or light_ws2812 (ATmega88P) wrapper for the single WS2812 LED on PD5. *(scenes)* `globalBrightness` multiplier on all output |
...
| **v4** | scenes | `PacketAnimationPrepare`: `colorFrom`/`colorTo` changed from `ColorRGB` (3 B) to `ColorRef` (4 B). Three new appearance packets. **Panel and controller must be flashed together.** |
...
| 17 | `SET_PALETTE` *(scenes)* | C→P or GC | 70 B | 16-stop gradient; GC = broadcast to all |
| 18 | `SET_BASE_COLORS` *(scenes)* | C→P or GC | 14 B | 3 × RGB base colors |
| 19 | `SET_GLOBAL_BRIGHTNESS` *(scenes)* | General Call | 6 B | 0–255 multiplier |
...
- `SET_PALETTE` / `SET_BASE_COLORS` / `SET_GLOBAL_BRIGHTNESS` *(scenes)*

Panels receive General Call only after `TWAR |= 0x01` (GCIE bit). START and UPDATE_PARAMS packets are sent **twice** (300 µs apart) with a `seq_id` duplicate guard so the panel processes exactly one copy.
...
- *(scenes)* `resolveColorRef()` called at the start of every tick function — palette/color changes take effect on the next frame with no re-prepare
...
Delta optimization: all runners cache `lastBrightness[]` per panel and only send I²C when the value changes. Typical cost: ≤ 600 µs/frame for a 3-wide wave.
...
```
Controller                    Panel
    │── HANDSHAKE (500 µs) ──►│   Panel detects pulse via PCINT
    │◄── HANDSHAKE (500 µs) ──│   ACK; enters STATE_REGISTER_EDGES; calls LNBus.begin(0x78)
    │◄── DONE (2000 µs) ──────│   Entire subtree has finished registering
```

The Panel ISR calls `LNPanel.updateEdgesStates((PINB >> 1) & 0x07, TCNT1)` directly. Timer1 runs free at prescaler 8 (0.5 µs/tick) for pulse-duration measurement. Controller uses `micros() * 2` for the same units.
...
Sequence after `LNPanelsInitializer.isReady() == true` (state machine `case 0`):

1. *(scenes)* Mount SPIFFS early — `SPIFFS.begin()` hoisted before WiFi so `PaletteStore` and `AppearanceStore` can read `/palettes/` and `/config/` before the captive-portal blocks
2. `sendConfiguration()` — broadcasts `PacketPanelConfiguration` (gamma, color temp) to all panels
3. `selfTest()` — fade-in/out on every panel in discovery order
4. *(scenes)* `PaletteStore::seedBuiltInsIfMissing()` — creates `/palettes/*.json` for all 8 built-ins on first boot
