...
public:
    ...
    void setPanelIndex(uint8_t idx) { panelIndex = idx; }

    void turnOn()  { if (!isOn) { isOn = true;  maybeLog(); } }
    void turnOff() { if (isOn)  { isOn = false; maybeLog(); } }

    bool on() const { return isOn; }

    Protocol::ColorRGB color() const { return colorValue; }
    ...
    void color(Protocol::ColorRGB* c) { color(c->r, c->g, c->b); }
