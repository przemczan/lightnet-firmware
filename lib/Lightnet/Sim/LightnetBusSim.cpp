...
uint8_t LightnetBus::sendPacket(uint8_t address, void* packet, uint8_t size,
                                 Protocol::packetType_t type, bool end) {
    Protocol::setPacketMeta(packet, type);
    return sendData(address, packet, size, end);
}

uint8_t LightnetBus::sendPacketNack(uint8_t addr, void* pkt, uint8_t size,
                                     Protocol::packetType_t t) {
    return sendPacket(addr, pkt, size, t, true);
}

uint8_t LightnetBus::sendPacketAck(uint8_t addr, void* pkt, uint8_t size,
                                    Protocol::packetType_t t) {
    sendPacket(addr, pkt, size, t, false);
    return 0;
}

uint8_t LightnetBus::sendPacketWithResponse(uint8_t addr, void* pkt, uint8_t pktSize,
                                              Protocol::packetType_t type,
                                              void* respBuf, uint8_t respSize) {
...
uint8_t LightnetBus::sendResponsePacket(void*, uint8_t, Protocol::packetType_t) { return 0; }
uint8_t LightnetBus::sendResponseData(void*, uint8_t) { return 0; }
uint8_t LightnetBus::requestData(uint8_t, void*, uint8_t) { return 0; }
uint8_t LightnetBus::requestPacket(uint8_t, void*, uint8_t) { return 0; }
