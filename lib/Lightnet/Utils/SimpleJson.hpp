...
inline void jsonFormatHex(uint8_t r, uint8_t g, uint8_t b, char* out)
{
    ...
    out[0]='#'; out[1]=hx[r>>4]; out[2]=hx[r&0xF];
    out[3]=hx[g>>4]; out[4]=hx[g&0xF];
    out[5]=hx[b>>4]; out[6]=hx[b&0xF];
    out[7]='\0';
}
...
{
    jsonSkipWs(p, end);
    if (end - p >= 4 && strncmp(p, "true", 4) == 0)  { *out = true;  p += 4; return true; }
    if (end - p >= 5 && strncmp(p, "false", 5) == 0) { *out = false; p += 5; return true; }
    return false;
}
...
inline bool jsonSkipValue(const char*& p, const char* end)
{
    ...
    if (*p == '{') { p++; return jsonSkipObject(p, end); }
    if (*p == '[') { p++; return jsonSkipArray(p, end); }
    while (p < end && *p != ',' && *p != '}' && *p != ']' &&
           *p != ' ' && *p != '\t' && *p != '\n' && *p != '\r') p++;
    return true;
}
...
public:
    ...
    const char* end()  const { return _body + _len; }
