#include "RelayBootloaderClient.hpp"
        ...
        bool RelayBootloaderClient::writeChunk(uint16_t panelIndex, uint16_t address, const uint8_t *data, uint8_t length)
        {
            ...
            if (!ok) {
                DEBUG_IF(DEBUG_FLASHER, D_PRINTFLN("[RELAY-BOOT] writeChunk timeout @ panel %u addr 0x%04X", panelIndex, address));

                return false;
            }

            if (ack.status != Protocol::BOOTLOADER_WRITE_OK) {
                DEBUG_IF(DEBUG_FLASHER, D_PRINTFLN(
                             "[RELAY-BOOT] writeChunk rejected @ panel %u addr 0x%04X status=%d",
                             panelIndex,
                             address,
                             ack.status
                ));
