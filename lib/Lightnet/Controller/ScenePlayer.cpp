...
ScenePlayer::ScenePlayer(AnimationScheduler& _scheduler,
...
void ScenePlayer::loadAndPlay(const SceneLayer* newLayers, uint8_t newCount,
                               bool newLoop, const char* newName,
    ...
    if (layer.targetMode == PanelTargetMode::ALL) {
        ...
        for (uint16_t i = 0; i < total && count < SCENE_MAX_PANELS_PER_LAYER; i++) {
            ...
            for (uint8_t j = 0; j < layer.targetCount; j++) {
                if (layer.targetList[j] == addr) { excluded = true; break; }
            }
