...
#include "../Controller/Scene/ScenePlayer.hpp"
#include "../Controller/Scene/SceneParser.hpp"
#include "../Controller/Scene/AnimationScheduler.hpp"
#include "../Controller/Scene/IPacketSink.hpp"
#include "../Controller/Scene/IPaletteResolver.hpp"
#include "../Controller/Scene/SceneTopology.hpp"  // ITopologyProvider, TopoLink
#include "../Controller/Scene/TagResolver.hpp"    // ITagResolver, TopologyIndex, PanelSet
...
namespace {
    ...
    struct RegPalettes : public IPaletteResolver {
        ...
        void add(const char *name, const GradientStop *stops, uint8_t count)
        {
            ...
            int slot = n;
            for (int i = 0; i < n; i++) if (strcmp(entries[i].name, name) == 0) { slot = i; break; }
            if (slot == n) n++;
    ...
    struct RegTags : public ITagResolver {
        struct Entry { char name[SC_NAME_LEN]; uint8_t panel; };
    ...
    struct RegTopology : public ITopologyProvider {
        ...
        {
            for (uint8_t i = 0; i < count; i++) { idx[i] = indices[i]; ec[i] = edgeCounts[i]; }
    ...
    struct SceneCore {
        ...
        void resetBatch(uint32_t now) { outbuf.clear(); outCount = 0; lastNow = now; }
    };

    inline SceneCore *self(scene_handle h) { return reinterpret_cast<SceneCore *>(h); }
}  // namespace
